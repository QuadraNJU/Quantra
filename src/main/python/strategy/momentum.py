...
def handle(account):
    ...
    momentum = pandas.DataFrame(momentum).sort_values(by='c_ret').reset_index()
    ...
    for stk in account.sec_pos:
        if stk not in buylist:
            account.trade(stk, 0)
    # 等权重买入所选股票
    portfolio_value = account.portfolio
    for stk in buylist:
        if stk not in account.sec_pos:
            account.trade(stk, int(portfolio_value / account.ref_price[stk] / len(buylist) / 100.0) * 100)
